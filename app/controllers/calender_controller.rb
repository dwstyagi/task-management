class CalenderController < ApplicationController
  ...
  def index
    ...
    @tasks = if current_user.organisation_owner?
      ...
      Task.where(assignee: current_user, due_date: start_date.beginning_of_month.beginning_of_week..start_date.end_of_month.end_of_week)
    end
  end
end
